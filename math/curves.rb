...
# Collects a number of curves suitable for changing a value over time. These may
# be useful for functions like {Track#with_vel_curve}.
#
...
module Curves
  ...
  # A curve that moves quadratically from 1 to 0 at the halfway point, then back
  # to 1.
  DownUpQuad = ->(x) { (2.0 * x - 1.0) ** 2 }
  # A curve that moves quadratically from 0 to 1 at the halfway point, then back
  # to 0.
  UpDownQuad = ->(x) { 1.0 - (2.0 * x - 1.0) ** 2 }

  # A curve that moves cubically from 0 to 1. This is one arm of `x^3`; there is
  # no leveling out in the middle.
  UpCubic = ->(x) { x ** 3 }
  # A curve that moves cubically from 1 to 0. This is one arm of `x^3`; there is
  # no leveling out in the middle.
  DownCubic = ->(x) { -((x - 1.0) ** 3) }

  # A curve that moves cubically from 0 to 1. This is the full cubic curve, and
  # has a leveling in the middle.
  UpFullCubic = ->(x) { (Math.cbrt(4.0) * x - Math.cbrt(0.5)) ** 3 + 0.5 }
  # A curve that moves cubically from 1 to 0. This is the full cubic curve, and
  # has a leveling in the middle.
  ...
  # A curve that moves along a sine from 1 to 0 at the halfway point, then back
  # to 1.
  DownUpSine = ->(x) { Math.cos(2.0 * Math::PI * x) / 2.0 + 0.5 }
  # A curve that moves along a sine from 0 to 1 at the halfway point, then back
  # to 0.
  ...
  #
  # @param f [#call] The curve to scale, which should be callable with a single
  ...
  # @return [#call] A lambda that will call `f` and scale its output so that it
  ...
  # over the range of 0 to 1. `ramp_up_start` determines at what input value the
  # increase will begin; the function will return `min_value` for inputs below
  # `ramp_up_start`, and then linearly increasing values between `min_value`
  # and `max_value` for those after.
