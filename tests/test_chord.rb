...
class ChordTest < Test::Unit::TestCase
  ...
  def _test_one_diatonic_degree(chord_root, chord_name, degree, tonic, scale_name, degree_rotation)
    ...
    if ExtApi.in_sonic_pi?
      1.upto(7) do |i|
        spi_notes = ExtApi.chord_degree(degree, tonic, scale_name, i).to_a.map { |n| N(n) }
        ...
        1.upto(i - 1) do |invert|
          spi_notes = ExtApi.chord_degree(degree, tonic, scale_name, i, invert: invert).to_a.map { |n| N(n) }
