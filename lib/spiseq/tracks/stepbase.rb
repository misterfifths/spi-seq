...
require_relative "../theory/midinote"  # just for a check in repr
...
module SpiSeq; module Tracks
  ...
  #   `repr_ctor_method` so that {#repr} and `mutate` work as expected. They
  #   must provide `default_accum_target` and `valid_accum_targets` to handle
  #   initialization of accumulation parameters. And `unique_slot_key` must
  #   be implemented to provide a key that uniquely identifies a step among
  #   other steps in the same slot.
  class StepBase
    ...
    # {#accum_min} and {#accum_max} (exclusive). If the delta is 0 (the
    # default), no accumulation occurs.
    #
    # The accumulated value for a step is used in different ways depending on
    # the subclass. For {Step}s, it applies a semitone offset to the {Step#note
    # note}, or a shift to the {Step#vel velocity} or {Step#gate gate}; for
    # {CCStep}s it applies an offset to the {CCStep#value value} that will be
    # sent for the CC. {#accum_target} determines what parameter accumulation
    # will effect.
    #
    # @return [Number]
    attr_reader :accum_delta

    # The acceptable range of the total accumulation, inclusive. The behavior
    # when the total exceeds one of these values is controlled by `accum_mode`.
    # The default range is 0 - 12. `accum_min` must be <= 0, and `accum_max`
    # must be greater than `accum_min`.
    # @return [Number]
    ...
    #   would result in an accumulation of 14, `:wrap` mode will convert that
    #   into an accumulation of `accum_min + 1`.
    ...
    # A predicate that determines whether accumulation will trigger when the
    # step does, or nil if it should always trigger. Accumulation can only
    # trigger when the step itself does. That is, the overall step {#prob} (if
    # any) is a prerequisite for the evaluation of `accum_prob` and the
    # application of accumulation at all.
    # @see #prob
    # @return [Prob, nil]
    attr_reader :accum_prob

    # Defines the target of accumulation on this step. The valid values depend
    # on the subclass. For {CCStep}s, the only option is `:value`; accumulation
    # will effect {CCStep#value}. For {Step}s, it may be `:note`, `:gate`, or
    # `:vel`, which will target {Step#note}, {Step#gate}, and {Step#vel}
    # respectively.
    # @return [Symbol]
    ...
    # by some accumulating amount each time it triggers. The actual effect of
    # the accumulation varies by subclass and is determined by `accum_target`.
    # For {Step}s, accumulation applies a semitone offset to the {Step#note
    # note}, or shifts the {Step#gate gate} or {Step#vel velocity}. For
    # {CCStep}s it applies an offset to the {CCStep#value value} that will be
    # sent for the CC.
    #
    ...
    # @param accum_delta [Integer] The amount to add to the accumulation each
    #   time the step triggers during playback; see {#accum_delta}.
    ...
    #   {#accum_min}.
    # @param accum_mode [:freeze, :wrap, :reverse] Controls the behavior when
    #   the total accumulation exceeds the range defined by `accum_min` and
    ...
    # @param prob [Prob, Number, #call, nil] See {StepBase#accum_prob}.
    #   Non-{Prob} values will be converted to {Prob}s as described by
    #   {StepBase#initialize}.
    ...
    # @param float_digits [Integer] The number of digits to show after the
    ...
    # from each key will result in a keyword argument to the `accum` method
    # (with the exception of `accum_delta` which is assumed to be positional).
    # Used to implement `repr`, `mutate`, equality and hashing.
    def accum_kwargs
      {
        accum_delta: 0,
        accum_max: 12,
