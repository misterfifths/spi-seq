...
module SpiSeq; module Tracks
  # A Step represents a sequenceable MIDI note event which can appear in slots
  # in the grid of a {Track}. See the documentation for {StepBase} for details
  # about steps in general, and their probability and accumulation mechanics.
  #
  ...
  class Step < StepBase
    ...
    # @return [Theory::MIDINote]
    attr_reader :note

    # The velocity to use when playing this step over MIDI, 0 - 127 inclusive.
    #
    # When played with Sonic Pi's internal synthesis, the velocity will
    # translate into an `amp` value passed to `play`.
    #
    # @return [Integer]
    attr_reader :vel

    # The gate for this step as a percentage 0.0 - 1.0. This is the percentage
    # of the duration of a slot for which the step will trigger its note. For
    ...
    # @param note [Theory::MIDINote, String, Symbol, Integer] The note this step
    #   will play when triggered. May be any value understood by
    #   {Theory::MIDINote.new}.
