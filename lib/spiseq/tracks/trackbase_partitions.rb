...
module SpiSeq; module Tracks
  ...
  class TrackBase
    # @!macro [new] drop_desc
    #   If `drop` is false, both tracks will have the same length; slots that
    #   are selected into one track will have a corresponding rest in the other.
    #   If `drop` is true, that is not the case: the first track will contain
    #   only the selected slots and the second the remaining ones. If that
    #   operation would result in a track with no slots, nil is returned for
    #   that track.
    #   <!-- forcing a newline -->
    ...
    # equivalent of {#partition_steps}, except that by default the returned
    # tracks may not have the same length as this track.
    #
    # @macro drop_desc
    #
    ...
    # {#reject_slots}. To clear select slots, rather than removing them
    # entirely, see {#clear_slots} and {#clear_slots_except}.
    #
    # @example
    #   t = T[[:c1, :c2], :d2, :e2, :f2]
    #
    ...
    #   slot.
    # @yieldreturn [Boolean] If true, the slot will be placed in the first of
    ...
    def partition_slots(drop: true, &block)
      ...
      @grid.each_with_index do |slot, i|
        if Internal::Utils.call_varargs(block, slot, i)
          grid1 << slot
          grid2 << [] unless drop
        else
          grid1 << [] unless drop
    ...
    #    length, or a range, in the same manner as `Array#slice`. Selecting
    #    slots this way is equivalent to using {#slice}.
    ...
    # {#partition_slots}. This method's complement is {#reject_slots}. To
    # convert unselected slots to rests, rather than removing them entirely, use
    ...
    #   select, or a range of indexes to select. Invalid indexes are ignored. It
    #   is an error to provide this value if a block is also provided.
    # @param length [Integer, nil] If `idx_or_range` is an integer, the number
    #   of slots after that index to select (nil will select just that slot). It
    #   is an error to provide a value for this parameter if the first argument
    #   is a range or nil.
    ...
    #
    # This is the complement of {#select_slots} and behaves exactly like it
    # except that the criteria for selecting slots is inverted. If given a
    # block, it returns exactly the second track from a call to
    # {#partition_slots}. To convert unselected slots to rests, rather than
    # removing them entirely, use {#clear_slots}.
    #
    ...
    #
    # @param idx_or_range [Integer, Range] The index of the first slot to
    #   reject, or a range of indexes to reject. Invalid indexes are ignored. It
    #   is an error to provide this value if a block is also provided.
    # @param length [Integer, nil] If `idx_or_range` is an integer, the number
    #   of slots after that index to reject (nil will reject just that slot). It
    #   is an error to provide a value for this parameter if the first argument
    #   is a range or nil.
    # @return [TrackBase, nil]
    # @see #partition_slots
    ...
    # `drop` set to false. To remove slots rather than convert them to rests,
    # use {#reject_slots}.
    #
    # If neither arguments nor a block is passed, all slots are cleared in the
    # new track.
    #
    # @example
    #   t = T[[:c1, :c2], :d2, :e2, :f2]
    ...
    #
    # @yieldparam (see #partition_slots)
    # @yieldreturn [Boolean] If true, the corresponding slot will be a rest in
    #   the returned track. If false, the new track will contain the slot as-is.
    #
    ...
    # @param length [Integer, nil] If `idx_or_range` is an integer, the number
    #   of slots after that index to clear (nil will clear just that slot). It
    #   is an error to provide a value for this parameter if the first argument
    #   is a range or nil.
    # @return [TrackBase]
    # @see #partition_slots
    ...
    #
    # This is the complement of {#clear_slots} and behaves exactly like it
    # except that the criteria for selecting slots is inverted. If given a
    # block, it returns exactly the first track from a call to
    # {#partition_slots} with `drop` set to false. To remove slots except for
    # certain ones, rather than convert them to rests, use {#select_slots}.
    #
    # @example
    #   t = T[[:c1, :c2], :d2, :e2, :f2]
    #
    ...
    #
    # @param idx_or_range [Integer, Range] The index of the first slot to
    #   maintain, or a range of indexes to maintain. Invalid indexes are
    #   ignored. It is an error to provide this value if a block is also
    #   provided.
    # @param length [Integer, nil] If `idx_or_range` is an integer, the number
    #   of slots after that index to maintain (nil will maintain just that
    #   slot). It is an error to provide a value for this parameter if the first
    #   argument is a range or nil.
    # @return [TrackBase]
    # @see #partition_slots
    # @see #select_slots
    # @see #clear_slots
    ...
    # {#reject_every}. To clear select slots, rather than removing them
    # entirely, see {#clear_every} and {#clear_every_except}.
    #
    # @example
    ...
    #     :r, :c4]
    #
    # @param gaps [Integer] Specifies the slots to clear; see above. You must
    ...
    # Returns a new track containing only slots that are certain distances
    # apart.
    #
    # If no slots are selected, returns nil.
    #
    # This returns exactly the first track from a call to {#partition_every}.
    # This method's complement is {#reject_every}. To convert unselected slots
    # to rests, rather than removing them entirely, use {#clear_every_except}.
    # To clear slots this one would select, use {#clear_every}.
    #
    # @example
    ...
    #   T[:b2, :e1, :g1]
    #
    # @param gaps [Integer] Specifies the slots to select; see
    ...
    #   T[:a1, :c1, :d1, :f1]
    #
    # @param gaps [Integer] Specifies the slots to reject; see
    #   {#partition_every}. You must pass at least one value.
    # @macro skip_empty_param
    # @return [TrackBase, nil]
    # @see #partition_every
    ...
    # {#clear_every_except}. To remove slots rather than converting them to
    # rests, use {#reject_every}.
    #
    # @example
    #   t = T[:a1, :b1, :c1, :d1, :e1, :f1, :g1]
    #
    ...
    #   T[:a1, :r, :c1, :d1, :r, :f1, :r]
    #
    # @param gaps [Integer] Specifies the slots to clear; see
    #   {#partition_every}. You must pass at least one value.
    # @macro skip_empty_param
    # @return [TrackBase]
    # @see #partition_every
    ...
    # Returns a new track clearing slots except those are certain distances
    # apart.
    #
    # This is the complement of {#clear_every} and behaves exactly like it
    # except that the criteria for selecting slots is inverted. It returns
    # exactly the first track from a call to {#partition_every} with `drop` set
    # to false. To remove the slots that this method converts to rests, use
    # {#select_every}.
    #
    # @example
    #   t = T[:a1, :b1, :c1, :d1, :e1, :f1, :g1]
    ...
    # {#select_x_of_y} and {#reject_x_of_y}. To clear slots, rather than
    # removing them entirely, see {#clear_x_of_y} and {#clear_except_x_of_y}.
    #
    # @example
    #   t = T[:a1, :b1, :c1,
    #         :d1, :e1, :f1]
    ...
    # Returns a new track containing only slots in the `x`th slot of each group
    # of `y`.
    #
    # If no slots are selected, returns nil.
    #
    # This returns exactly the first track from a call to {#partition_x_of_y}.
    # This method's complement is {#reject_x_of_y}. To convert unselected slots
    # to rests, rather than removing them entirely, use {#clear_except_x_of_y}.
    # To clear slots this method would select, use {#clear_x_of_y}.
    #
    # @example
    #   t = T[:a1, :b1, :c1,
    #         :d1, :e1, :f1]
    ...
    # Returns a new track excluding slots in the `x`th slot of each group of
    # `y`.
    #
    # If all slots are rejected, returns nil.
    #
    ...
    #
    # This method behaves exactly like {#reject_x_of_y} except that the length
    # of the track is maintained; selected slots are not removed, they are
    # instead replaced with rests.
    #
    # This returns exactly the second track from a call to {#partition_x_of_y}
    # with `drop` set to false. This method's complement is
    ...
    # slots.
    #
    # This is the complement of {#clear_x_of_y} and behaves exactly like it
    # except that the criteria for selecting slots is inverted. It returns
    # exactly the first track from a call to {#partition_x_of_y} with `drop` set
    # to false. To remove slots that this method converts to rests, use
    # {#select_x_of_y}.
    #
    # @example
    #   t = T[:a1, :b1, :c1,
    ...
    # If no slots are selected, returns nil.
    #
    # This returns exactly the first track from a call to {#partition_rand}.
    # This method's complement is {#reject_rand}. To convert unselected slots to
    # rests, rather than removing them entirely, use {#clear_except_rand}. To
    # clear slots this method would select, use {#clear_rand}.
    #
    # @example
    #   t = T[:a1, :b1, :c1, :d1, :e1]
    #
    ...
    #
    # This returns exactly the second track from a call to {#partition_rand}.
    # This method's complement is {#select_rand}. To convert selected slots to
    # rests, rather than removing them entirely, use {#clear_rand}.
    #
    # @example
    #   t = T[:a1, :b1, :c1, :d1, :e1]
    #
    ...
    # This returns exactly the second track from a call to {#partition_rand}
    # with `drop` set to false. This method's complement is
    # {#clear_except_rand}. To remove slots rather than converting them to
    # rests, use {#reject_rand}.
    #
    # @example
    #   t = T[:a1, :b1, :c1, :d1, :e1]
    #
    ...
    #
    # @param p [Number] The probability that any given slot will not be cleared
    #   in the result, 0 - 1 inclusive.
    # @return [TrackBase]
    # @see #partition_rand
    # @see #select_rand
    # @see #clear_rand
