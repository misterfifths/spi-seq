...
module SpiSeq; module Tracks
  class Track < TrackBase
    ...
    # element array, the first element of which is an integer number of tied
    # steps, and the second of which is the floating point gate for the final
    # step after the ties. If the duration maps entirely to ties, the second
    # element is 0. If it maps to a single step that is not a tie, the first
    # element is 0. E.g., a return of [15, 0.1] means the duration occupies 15
    # tied steps followed by one step with a gate of 0.1. A return of [10, 0.0]
    # means the duration occupies exactly 10 tied steps. And [0, 0.5] means the
    # duration occupies one step with a gate of 0.5.
    #
    # If there is at least one tie and the final step would have a gate less
    # than min_gate, it is ignored, and the second element of the return will be
    # 0. However, if the duration only maps to one step, and that step's gate
    # would be less than min_gate, this function returns [0, min_gate]. E.g., if
    # the return would be [15, 0.01], but min_gate is 0.1, the function will
    # return [15, 0] instead. But if the return would be just [0, 0.01] and the
    # min_gate is 0.1, the function will return [0, 0.1]. This ensures that very
    # short durations are not lost entirely. The function always returns [0,
    # min_gate] for a duration of 0.
    #
    ...
    private_class_method def self.gates_for_duration(duration, secs_per_slot,
      ...
      if float_lt(total_gate, 1.0)
        ...
        # If we happened to be very close to the threshold for 1.0, but not
        # quite, round up.
    ...
    # Add Steps to an array of slots for a note being played for the given
    # period. The elements of slots must be arrays (they will be appended to),
    # and slots itself must be long enough to hold the steps that the note event
    # maps to. This function considers the slots to begin at time 0, so
    # start_time and end_time should be relative to that.
    #
    ...
    private_class_method def self.add_note_to_slots(slots, secs_per_slot, note,
      ...
      # If we wound up with a zero duration from the snapping,
    ...
    #
    # The timeline is an array of four-element arrays, the elements of which
    # are:
    ...
    # must fall between those times. If either is nil, that endpoint is
    # determined from the events in the timeline. I.e., if start_time is nil,
    # the start time of the first event in the timeline is used. Likewise if
    # end_time is nil, the greatest end time among all the events in the
    # timeline is used.
    #
    ...
    # how to map between seconds in the timeline and steps in the track.
    # Granularity can be a NoteLength instance or any of the values accepted by
    # NoteLength.new. If bpm is nil, the current Sonic Pi BPM is used.
    #
    ...
    def self.from_timeline(timeline,
      ...
      if trim_end
        ...
        # for an additional slot to allow events that would get snapped up to
        # the next slot and then need their duration rounded to play out. E.g.
        # if we have 1s/slot and something goes from 0.5 - 1, we want to let
        # that snap to slot 1 and get min_gate. We'll trim the excess later.
      ...
      timeline.each do |entry|
        ...
        if note_end <= note_start
          Internal::Log.warn("timeline event ends before it starts or has 0 duration; ignoring", "recorder")
          next
        end
      ...
      # Also we purposefully added padding at the end in that case. So trim up
      # the final track if need be.
    ...
    # cc_channel, either of which may be wildcards. The value of the CC message
    # is ignored.
    #
    # MIDI notes from the given port and channel (which may also be wildcards)
    # are recorded in the timeline. Any notes that are still on when recording
    # is stopped by the control_cc are treated as if they ended at the same time
    # that the CC was received.
    #
    # This method blocks the calling thread until recording is stopped via the
    # CC. It must be called in a real-time context, i.e. in a thread that has
    # called use_real_time, or in a with_real_time block.
    #
    # The return is a three-element array [start time, end time, timeline],
    # where the timeline is as described in from_timeline.
    private_class_method def self.record_timeline(control_cc:, cc_port:, cc_channel:,
      ...
      # If these are nil for the default, we need to actually retrieve the
      # default since we're going to be doing string comparisons against the
      # port/channel.
      port, channel = Internal::MIDI.resolve_port_and_channel(port, channel)

      event_re = %r{^/midi:(?<port>[^:]+):(?<channel>\d+)/(?<event>.+)$}
      event_glob = "/midi:*/{control_change,note_on,note_off}"
      loop do
        note_or_cc, vel_or_cc_val = External::Sync.sync(event_glob)

        # get_event is undocumented, but it gives back a CueEvent object for the
        # most recent thing you sync'd to, given the argument you passed to
        # sync. The path property on that object is the string for the event.
        cue_event = External::Sync.get_event(event_glob)
        cue_name = cue_event.path

        # cue_event.time.to_f is also an option
        cue_time = External::Sync.vt
        ...
        if cue_event == "control_change"
          ...
          if recording
            Internal::Log.log("ending recording @ #{cue_time}", "recorder")

            recording = false
            end_time = cue_time
            break
          else
            Internal::Log.log("starting recording @ #{cue_time}", "recorder")
          ...
          note = Theory::MIDINote.new(note_or_cc)
          vel = vel_or_cc_val
          ...
          if cue_event == "note_on"
            if active_event.nil?
              ...
              Internal::Log.warn("got a note on for #{note}, but it's already on", "recorder")
            end
          elsif active_event.nil?
            Internal::Log.warn("got a note off for #{note}, but we didn't see it come on", "recorder")
          else
    ...
    # to a grid.
    #
    # Recording is stopped and started by a MIDI CC (the `cc` argument). The
    # value of the CC message is ignored. Any notes that are still on when
    # recording is stopped by the CC are treated as if they ended at the same
    # time that the CC was received.
    #
    # `bpm` is the beats per minute at which the track is to be played back, and
    # `granularity` is the slot granularity for the resulting track. Together
    # these two determine the duration of a slot in the track, and thus control
    # how to map between wall-clock seconds and steps in the track. A shorter
    # granularity - or a faster BPM - means that the timing of incoming MIDI
    # notes can be represented more precisely, which may or may not be
    # desirable.
    #
    # By default, the track will last for the duration that recording was
    # enabled via the CC; if there was silence at the beginning or end of
    # recording, there will be rests on either end of the resulting track. If
    # true, `trim_start` and `trim_end` will remove those rests from the
    # corresponding ends of the track.
    #
    # If for any reason the resulting track would be empty (e.g. if no MIDI
    # notes were observed while recording), this function returns nil.
    #
    ...
    #   nil, falls back to the global default set with
    #   {Utils::MIDI.use_cc_control_defaults} or all ports (i.e. "*") if no
    #   default was set.
    # @param cc_channel [Integer, String, nil] The MIDI channel to monitor for
    #   CC messages. If nil, falls back in the same manner as `cc_port`.
    # @param port [String, nil] The MIDI port to monitor for notes. If nil,
    #   falls back to the global default set with Sonic Pi's
    #   `use_midi_defaults`, or all ports (i.e. "*") if no default was set.
    # @param channel [Integer, String, nil] The MIDI channel to monitor for
    #   notes. If nil, falls back in the same manner as `port`.
    ...
    #   Pi BPM.
    # @param granularity [Symbol, Number, Theory::NoteLength] The granularity of
    #   the resulting track. Can be a {Theory::NoteLength} or a value understood
    ...
    #   will be ignored and all steps in the track will have the default
    #   velocity of 127.
    # @return [Track, nil]
