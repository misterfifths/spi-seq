...
module SpiSeq; module Tracks
  ...
  class CCTrack < TrackBase
    ...
    # @private
    def self.stepify(x)
      raise ArgumentError, "A rest cannot be converted to a step" if Theory::MIDINote.rest?(x)
    ...
    # CCSteps). The returned array will be frozen. Conversion rules:
    ...
    # @private
    def self.slotify(x)
      if Theory::MIDINote.rest?(x)
        [].freeze
      elsif x.is_a?(CCStep)
        [x].freeze
      elsif Internal::Enumerables.enumerable?(x)
        # See the note in enumerable? about arrayify.
        raw_slot = Internal::Enumerables.arrayify(x).reject { |s| Theory::MIDINote.rest?(s) }.map { |s| stepify(s) }
    ...
    def self.gridify(x)
      if Theory::MIDINote.rest?(x)
        ...
        # See the note in enumerable? about arrayify.
        Internal::Enumerables.arrayify(x).map { |s| slotify(s) }.freeze
      else
