...
module SpiSeq; module Internal; module ThreadTracker
  ...
  # Returns the Thread object associated with the the given name. If the thread
  # with that name is not running or has not been registered, returns nil.
  ...
  module_function def var_set(thread_name, var_name, value)
    get_thread(thread_name)&.thread_variable_set(var_name, value)
  end
  ...
  module_function def var_get(thread_name, var_name)
    get_thread(thread_name)&.thread_variable_get(var_name)
  end
end; end; end
