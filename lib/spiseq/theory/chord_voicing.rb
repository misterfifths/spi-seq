...
module SpiSeq; module Theory
  class Chord
    ...
    #   only interval in the chord is the root, this voicing will return an
    #   empty array.
    # - `shell`: Only the root, thirds, and seventh intervals (in any octave)
    #   are included. If none of those intervals are present in the chord, this
    #   voicing will return an empty array.
    # - `drop2`: Applies a closed voicing, then drops the 2nd highest note in
    #   the result an octave.
    ...
    # - `double3`: Applies a closed voicing, then adds a note an octave below
    #   any major or minor 3rd (in all octaves).
    ...
    #   octave.
    #
    # There are aliases for many of the above styles; print this array to see
    # all possible names.
    #
    # @return [Array<Symbol>]
    ...
    # @param root [MIDINote, String, Symbol, Integer] The root note of the
    #   chord. May be a {MIDINote} or anything understood by {MIDINote.new}.
    ...
    #   by {Chord#voice}.
    # @param num_octaves [Integer] How many octaves of the chord to express.
    #   This many copies of the chord's intervals will be added, each an octave
    #   higher, before inverting and voicing.
    ...
    # helper function.
    #
    # @param root [MIDINote, String, Symbol, Integer] The root note upon which
    #   to voice the chord. Must be a {MIDINote} or something understood by
    #   {MIDINote.new}.
    # @param voicing [Symbol] The voicing style to use. Valid values are listed
    #   in the {.VOICING_STYLES} array.
    # @param num_octaves [Integer] How many octaves of the chord to express.
    #   This many copies of the chord's intervals will be added, each an octave
    #   higher, before inverting and voicing. Combining this with inversion is
    ...
    # Returns a set of notes for a chord of the given degree on a scale. The
    # root note of the chord is the note at degree `d` on the scale starting at
    ...
    #   a Roman numeral string or symbol (e.g. `:ii` or "ix"). See
    #   {Scale#degree} for details.
    # @param tonic [MIDINote, String, Symbol, Integer] The root note of the
    #   scale. May be a {MIDINote} or any value understood by {MIDINote.new}.
    # @param scale_name [Symbol, String] The name of the scale to use, one of
    #   the values in {Scale.SCALE_NAMES}. Most of the scale name recognized by
    #   Sonic Pi's `scale` function are supported, and some others.
    ...
    def self.degree(d, tonic, scale_name, number_of_notes = 4, voicing = :closed, invert: 0)
      ...
      # We may have wound up with fewer notes than `number_of_notes`, so
    ...
    # Doubles the lowest note in the closed voicing, offset by the given number
    # of semitones.
    ...
    # Doubles all notes corresponding to the given intervals (double_ints),
    # offset by the given number of semitones. Equivalent to a closed voicing if
    # none of the target intervals are in the chord.
    ...
    # Raise the 2nd lowest note an octave. Does nothing if there is only one
    # note.
    ...
    # Lower the 2nd lowest note an octave. Does nothing if there is only one
    # note.
