...
module SpiSeq; module Theory
  # A grouping of notes that represents some {#num_octaves number of octaves} of
  # a {#name scale}, starting on a particular {#tonic root note}. Enumerable
  # over its {#notes}, and has most of the read-only methods of Array.
  #
  # If you would like a scale that covers the full MIDI range, see
  # {.full_scale}. Such a scale is useful for, e.g., {Tracks::Track#scale}.
  #
  # Scales are immutable.
  #
  # Sonic Pi already provides a class called `Scale`, so this class is aliased
  # to {Sc}. Alternatively, you can use the {SC} function as an alias for
  ...
  class Scale
    ...
    SCALES = lambda do
      ...
      scales = {
        ...
        melodic_minor_asc:  :melodic_minor,
        melodic_min_asc:    :melodic_minor,
        hungarian_minor:    [2, 1, 3, 1, 1, 3, 1],
        hungarian_min:      :hungarian_minor,
        diminished:         [1, 2, 1, 2, 1, 2, 1, 2],
        diminished2:        [2, 1, 2, 1, 2, 1, 2, 1],
        octatonic:          :diminished2,
        messiaen1:          :whole_tone,
        ...
        melodic_minor_desc: :minor,
        melodic_min_desc:   :melodic_min_desc,
      ...
      # Resolve aliases
      scales.each do |name, val|
        scales[name] = scales[val] if val.is_a?(Symbol)
      end
    ...
    # The names of all scales supported by this class. These are the valid
    # values to pass to {#initialize}.
    #
    ...
    # `minor_pentatonic`, `yu`, `major_pentatonic`, `gong`, `egyptian`,
    # `shang`, `jiao`, `pentatonic`, `zhi`, `ritusen`, `whole_tone`, `whole`,
    # `chromatic`, `harmonic_minor`, `melodic_minor_asc`, `hungarian_minor`,
    # `octatonic`, `messiaen1`, `messiaen2`, `messiaen3`, `messiaen4`,
    # `messiaen5`, `messiaen6`, `messiaen7`, `super_locrian`, `hirajoshi`,
    # `kumoi`, `neapolitan_major`, `bartok`, `bhairav`, `locrian_major`,
    # `ahirbhairav`, `enigmatic`, `neapolitan_minor`, `pelog`, `augmented2`,
    # `scriabin`, `harmonic_major`, `melodic_minor_desc`, `romanian_minor`,
    # `hindu`, `iwato`, `melodic_minor`, `diminished2`, `marva`,
    # `melodic_major`, `indian`, `spanish`, `prometheus`, `diminished`, `todi`,
    # `leading_whole`, `augmented`, `purvi`, `chinese`, `lydian_minor`.
    #
    # There are also aliases for some of the above names; print this array to
    # see all possibilities. This class supports all of the integral scales that
    # Sonic Pi does; it does not handle the Turkish scales with fractional
    # intervals.
    #
    # @return [Array<Symbol>]
    ...
    #
    # @param tonic [MIDINote, String, Symbol, Integer] The root note of the
    #   scale. A {MIDINote} or one of the values understood by {MIDINote.new}.
    # @param name [Symbol, String] The name of the scale to use, one of the
    ...
    # @param clamp_to_midi [Boolean] If true, only notes in the MIDI range of
    #   0 - 127 will be included.
    ...
    #   the values in {.SCALE_NAMES}.
    # @return [Scale]
    # @see Tracks::Track#scale
    ...
    # `relative_tonic`. Raises an ArgumentError if `relative_tonic` is not in
    # the scale or if the scale does not contain a note that is `steps` far away
    # from it.
    ...
    # Returns the note in the scale that is `d` degrees from `relative_tonic`.
    # If `relative_tonic` is nil (the default), the tonic of the scale will be
    # used instead. Raises an ArgumentError if the scale does not contain
    ...
    # not the tonic of the scale. A degree of -1 is taken to be one note below
    # the tonic on the scale.
    #
    # Unlike Sonic Pi's `degree` function, this will only return notes within
    # the octave range of the scale on which it is called. See the class method
    ...
    #   details.
    # @param tonic [MIDINote, String, Symbol, Integer, nil] The root of the
    #   scale. May be a {MIDINote} or anything understood by {MIDINote.new}.
    # @param scale_name [Symbol, String] The name of the scale to use, one of
    #   the values in {.SCALE_NAMES}. This class understands most of the same
    #   scale names as Sonic Pi's `scale` function and some it doesn't.
    # @return [MIDINote]
    # @see .full_scale
    # @see #degree
    ...
    # Returns the number of steps on the scale between the given notes. Raises
    # an ArgumentError if either note is not in the scale.
    # @param relative_tonic [MIDINote, String, Symbol, Integer]
