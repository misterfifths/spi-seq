...
module SpiSeq; module Theory
  ...
  module Arp
    ...
    # An arpeggiation direction that returns notes in the same order as
    # {.UpDown}, but with each note doubled. The middle note is not repeated,
    # nor is the final note.
    #
    ...
    # An arpeggiation direction that returns notes in ascending order, but with
    # the lowest note in every other position. The low note is not doubled at
    # the beginning.
    #
    ...
    # An arpeggiation direction that returns notes in an order that climbs to
    # the highest note and then descends. The notes are sorted ascending, then
    # every other note is chosen to walk toward the highest note, then the
    # remaining notes appear in descending order.
    #
    #   arp([:c1, :c3, :c2, :c4, :c5], :peak)
    #   # returns [:c1, :c3, :c5, :c4, :c2]
    Peak = :peak

    # An arpeggiation direction that returns notes in an order that descends
    # from the highest note and then ascends. The highest note appears first,
    # then every other note descending to the lowest, then the remaining notes
    # in ascending order.
    #
    ...
    # returned as passed to {.arpeggiate}. This is not particularly useful on
    # its own, but may be together with `spread` or `extra_octaves`, which add
    # extra notes to the input.
    Order = :order
    ...
    # the duplicates will not be added to the result. In the {Arp::Order :order}
    # direction, the notes from the `spread` will appear at the end, in order
    ...
    # result, before notes from the `spread`. Like `spread`, `extra_octaves`
    # will not add duplicate notes.
    #
    # @example
    ...
    #   e.g. {Arp::Pinky :pinky} or {Arp::UpDown :updown}. You can pass the
    #   symbol directly rather than referencing the constant on the Arp module.
