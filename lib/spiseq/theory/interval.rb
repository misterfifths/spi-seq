...
module SpiSeq; module Theory
  ...
  #
  # Intervals are instances of Numeric, and their value is the number of
  # semitones they represent. That means you can add Intervals directly to
  # {MIDINote}s to obtain the note that is an interval away from another.
  #
  # Performing arithmetic on Intervals results in instances with a default
  # quality (major/minor/perfect when possible, but diminished for 6 semitones).
  # For instance, adding 1 to a major 2nd results in a minor 3rd, not an
  # augmented 2nd.
  #
  ...
  # a number of octaves ({#octave_span}) and the simple interval they represent
  # on top of that number of octaves ({#simple_interval}). When possible,
  ...
  class Interval < Numeric
    ...
    # The number of octaves this interval spans. Intervals that span more than
    # one octave are compound.
    # @return [Integer]
    # @see #simple_interval
    attr_reader :octave_span

    # If this interval spans more than one octave, this is the interval that
    # remains after those octaves. E.g. for a M10 interval, which is 16
    # semitones, this value is a M3, accounting for the 4 additional semitones
    # after the octave. If this interval is not compound this is just `self`,
    # except for P8 and A7 for which this is P1.
    # @return [Interval]
    # @see #octave_span
    # @see #simple?
    # @see #compound?
    ...
    # arithmetic or a size but no quality, the first quality is the one that
    # will be chosen for the new instance. E.g. `Interval.new(:P4) + 1` will
    # yield a diminished 5th, not an augmented 4th.
    SIZES_TO_NUMBERS = {
      0  => { perfect: 1, dim: 2 },
      1  => { minor:   2, aug: 1 },
      2  => { major:   2, dim: 3 },
    ...
    # [number of octaves, simple interval number]. For example, interval number
    # 10 decomposes into [1, 3] since it is one octave and a third.
    ...
    # - A symbol or string, which is taken as the abbreviated name of an
    #   interval.
    ...
    #
    # If given, quality must be one of `:major`, `:minor`, `:perfect`, `:aug`,
    # or `:dim`. Of course, not every combination number/size and quality is
    # valid - e.g. there is no such thing as a major 5th interval. This method
    # will raise if given an invalid quality.
    #
    ...
    private def initialize(size:, quality:)
      ...
      # Exact octave multiples should map to an 8th
      ...
      @simple_interval = if size > 0 && (size % 12) == 0
        # Exact multiples of an octave are funny. P8 & A7 are simple
        # (number < 8); d9 is technically compound though it's the same size. We
        # want all of them to have a simple_interval of a first so that they'll
        # be considered with things like shell chord voicing. So this is an
        # unfortunate special case where intervals with `simple?` true are not
        # their own simple_interval, and the qualities might not match.
        Interval.new(:P1)
      elsif num_octaves == 0
        self
      else
    ...
    def names
      ...
      # so undo that. We want the number of 7ths we need to add, and an index
      # into SIZES_TO_NUMBERS.
    ...
    # Returns true if this interval can be expressed as some quality of
    # `number`. A shortcut that checks if `as(number)` is not nil.
    # @param number [Integer]
    # @return [Boolean]
    # @see #as
...
#-
module SpiSeq; module Internal; module MonkeyPatches; module IntervalComparison
  ComparisonUtils.define_reverse_comparison_ops(self, Theory::Interval)
end; end; end; end

# @private
class Symbol
  prepend SpiSeq::Internal::MonkeyPatches::IntervalComparison
end
