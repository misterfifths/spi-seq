...
module SpiSeq; module Playback
  ...
  # A Player plays a {Tracks::Track} by sending its {Tracks::Step}s' notes over
  # MIDI or playing them with Sonic Pi's internal synthesis.
  #
  ...
  class Player < PlayerBase
    # Whether this player should send MIDI note events for {Tracks::Step}s, or
    # use Sonic Pi's internal synthesis instead.
    # @return [Boolean]
    attr_reader :midi
    ...
    # The MIDI port to use when this player sends events. Only relevant if
    # {#midi} is true.
    # @return [String, nil]
    attr_reader :port
    ...
    # Constructs a Player.
    #
    # @param track [Tracks::Track] The initial value for {#track}.
    ...
    # @param channel [Integer, String, nil] The MIDI channel to use when `midi`
    #   is true. If nil, falls back to the global default set by Sonic Pi's
    ...
    def initialize(track, midi: nil, channel: nil, port: nil, debug: false)
      ...
      # these, since they have a definite length that we can specify when
      # starting them; they'll terminate themselves.
    ...
    def accum_should_trigger?(step)
      ...
      # use effective_note here (which would take it into account). That means
      # that the `pre_same_note` family of probs won't work; they don't really
      # make sense on an accum anyway.
    ...
    # Deduplicate the given steps, which must come from the current slot, based
    # on their effective note (accounting for accumulation). In the case that
    ...
    #
    # The returned array of ended steps does not strictly contain steps that
    # ended exactly at the beginning of this step. It also contains steps that
    # ended between this step and the previous one - e.g. Steps with gates less
    # than 1.
    def categorize_steps(triggering_steps)
      ...
      # same slot, accumulation may result in duplicates. So we need to do
      # another deduplication pass here, based on the actual notes we'll be
      # playing.
      ...
      cur_steps.each do |step|
        ...
        # Steps with 0 gate do not exist from our perspective. They won't play
        # or continue ties.
        next if gate == 0
    ...
    def play_steps(steps)
      ...
      # as with categorize_steps, it is important that this method does not
      # assume anything about the order in which slots were or will be played.
      new_steps, tied_steps, ended_steps = categorize_steps(steps)

      if @debug
        Internal::Log.log("@ t=#{External::Sync.vt} slot=#{slot_idx} cycle=#{@cycle} fill=#{@fill}", "player")
        Internal::Log.log("new steps: #{steps_debug_string(new_steps)}", "player")
        Internal::Log.log("tied steps: #{steps_debug_string(tied_steps)}", "player")
      ...
      tied_steps.each do |step|
        ...
        External::Sync.at(gate * @track.granularity.to_f) do
          Internal::Log.log("killing #{step.inspect} @ t=#{External::Sync.vt}", "player") if @debug
          end_step(step)
        end
      end
    end
    ...
    def end_step(step)
      ...
      if @midi
        ...
        # removed.
        External::MIDI.midi_note_off(step_note, **@midi_spi_kwargs) unless @active_midi_notes.delete?(step_note).nil?
      else
        node = @active_synth_nodes.delete(step_note)
        External::Synth.kill(node) unless node.nil?
      end
    end

    def end_all_steps
      if @midi
        @active_midi_notes.each { |n| External::MIDI.midi_note_off(n, **@midi_spi_kwargs) }
        @active_midi_notes.clear
      else
        @active_synth_nodes.each_value { |node| External::Synth.kill(node) }
    ...
    # @track. Updates @active_midi_notes or @active_synth_nodes as needed to
    # track ongoing steps.
    def start_step(step)
      note, gate, vel = effective_attrs(step)

      if gate == 1
        # Step has indeterminate duration; it may be continued in the next
        # played slot. Start it and we'll kill it later when it ends in
        # play_steps.
        if @midi
          External::MIDI.midi_note_on(note, velocity: vel, **@midi_spi_kwargs)
          ...
          node = External::Synth.play(note, amp: vel / 127.0, sustain: @track.beat_length * 100)
