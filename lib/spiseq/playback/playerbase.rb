...
require_relative "../tracks/trackbase"
...
module SpiSeq; module Playback
  ...
  # subclasses of {Tracks::TrackBase}). Do not make instances of PlayerBase
  # directly. Instead use one of its subclasses: {Player} which handles note-
  # based {Track}s, or {CCPlayer} which handles {CCTrack}s. But even then, it is
  # unlikely you will make explicitly make a player. Instead use
  # {track_live_loop}, which will create and manage an appropriate player for
  # you.
  #
  ...
  #
  # The {#fill} attribute controls whether steps with the {Tracks::Prob.fill}
  # {Tracks::StepBase#prob probability} are played. It may be changed at any
  # point, even mid-cycle, and will take effect when the next slot is played.
  # When using a {track_live_loop} for playback, you can assign a MIDI CC that
  # will toggle fill mode with the `fill_cc` parameter.
  #
  # @abstract Subclasses must implement `play_steps`, `accum_should_trigger?`,
  #   and `step_should_trigger?`. If the subclass has additional internal state,
  ...
  class PlayerBase
    ...
    # @return [Tracks::TrackBase]
    attr_reader :track

    # The number of times this player has played a track. Incremented at the end
    # of every call to {#play}. Reset by {#stop}, and optionally by
    # {#swap_track}. A call to {#sleep} does not increment the cycle.
    # @return [Integer]
    attr_reader :cycle

    # Whether the player is in fill mode. In fill mode steps with a
    # {Tracks::StepBase#prob probability} of {Tracks::Prob.fill} will trigger,
    # and those with {Tracks::Prob.not_fill} will not. Fill mode can be toggled
    # at any point during playback and will take effect when the next slot is
    # played.
    # @return [Boolean]
    attr_accessor :fill

    # Creates a new PlayerBase that will play the given track.
    # @param track [Tracks::TrackBase] The initial value for {#track}.
    ...
    # sleeps (i.e. Sonic Pi's `sleep`) for the full
    # {Tracks::TrackBase#beat_length length} of the track. It honors the track's
    # {Tracks::TrackBase#timescale timescale} by internally applying a BPM
    # multiplier.
    #
    ...
    def play
      ...
      External::Sync.with_bpm_mul(@track.timescale) do
        @track.num_slots.times do |i|
          ...
          External::Sync.sleep(@track.granularity.to_f)
        end
      end
    ...
    # Sleeps (i.e. Sonic Pi's `sleep`) for the duration of the track. All
    # ongoing steps are stopped. The {#cycle} is not incremented.
    # @return [void]
    def sleep
      end_all_steps
      External::Sync.with_bpm_mul(@track.timescale) do
        External::Sync.sleep(@track.beat_length)
      end
    end

    # Swap out the track this player plays.
    #
    # This is intended to be called between calls to {#play} or {#sleep}. The
    # new track will take effect on the next `play` or `sleep`, beginning with
    # its first slot.
    #
    ...
    # is cleared (i.e., if you later swap back to the same track, the
    # accumulation of all of its steps will reset). If you call this with the
    # same track as {#track}, accumulation is not reset. You can override that
    # behavior by providing a value for `reset_accum`.
    #
    # @param new_track [Tracks::TrackBase] The track to use on the next call to
    #   {#play} or {#sleep}.
    ...
    # This is an internal method intended to be called when one player is
    # handing over playback to another, such as when a sketch is restarted.
    #
    ...
    #    evaluating probabilities. If it is needed to evaluate a probability,
    #    the `accum_delta` method may be used at this point to retrieve the
    ...
    #    trigger, as determined in step 4. Subclasses should take into account
    #    the now-commited accumulation for those steps, retrievable via
    #    `accum_delta`.
    ...
    # The next steps may not come from slot `slot_idx + 1`, and the previous
    # ones may not have come from slot `slot_idx - 1`. In fact they may not even
    # be from this track, if the track was swapped.
    ...
    # Called by `play` at the beginning of the process to play a slot.
    ...
    # Called by `play` at the beginning of the process to play a new slot,
    # before `slot_advanced`. Subclasses must implement this method by calling
    ...
    # method by calling `should_trigger?` on the step with as much information
    ...
    # `object_id` (Step, e.g., implements it by returning its note). So if a
    # track is swapped without resetting accum, we may reuse accumulation data
    # for non-identical steps. This is desirable in the case of
    # `track_live_loop`'s automatic fading: we want accumulation to persist
    # across non-identical tracks with non-identical steps.
    ...
    # Returns the new accumulation data entry for the step (in the current slot
    # of @track), if the step were to trigger in this cycle of playback. Does
    # not commit any changes to @accum_data. Evaluates the step's accum_prob
    # using step_accum_should_trigger? Returns nil if the step has no
    # accumulation. Returns the current data if the accumulation would not
    # trigger.
    def calculate_accum(step)
      ...
      # When the accum_delta is not an integer, we can't reasonably try to wrap
      # or reverse inclusively by subtracting one when we overstep the min or
      # max. See notes in test_accum.rb's test_float_step.
      ...
      if delta <= step.accum_min
        ...
        when :reverse
          # Always reverse direction, but only immediately apply a reversal if
          # we already stepped below the min. If we are exactly at the minimum,
          # then do not change the delta and wait for the next accumulation to
